...
# ╔═╡ bffa1f48-f5a8-11ed-01d6-f93515d22512
using Turing, OrdinaryDiffEq, StatsPlots, LinearAlgebra, Random, CSV, DataFrames

# ╔═╡ 73e9f1cb-0ffb-4c77-8148-e0cdeda0e123
begin 
	spikes_data = CSV.read("spikes_data.csv", DataFrame; header = [:t, :v])
	spikes_data = spikes_data[1:5:end, :]
	@. spikes_data.t = spikes_data.t * 7 + 1.3
	@. spikes_data.v = spikes_data.v - 0.1
	spikes_data  # print data
end

# ╔═╡ 2b9b1d44-daf7-430d-ba5d-023b656452d6
f(v, α) = v*(1-v)*(v-α)

# ╔═╡ f20f66c9-23ad-4236-8b6f-ddb04553303e
function fitzHugh_ode(du, u, p, t)
	# Model parameters.
	ε, α, γ, I_app = p
	# Current state.
	v, w = u
...
# ╔═╡ f8db467e-e801-4c64-afe3-9725132b8e8c
begin
	# Define initial-value problem.
	u0 = [1.0, 1.0]
	p = [0.01, 0.1, 0.5, 0.2]
	tspan = (0.0, 10.0)
	prob = ODEProblem(fitzHugh_ode, u0, tspan, p)
	
	# Plot simulation.
	plot(solve(prob, Tsit5()))
end

# ╔═╡ de713f15-484a-44b5-87b9-bfb38cbcc042
@model function fitlv(data, prob, ts)
    # Prior distributions.
    σ ~ InverseGamma(2, 4)
    α ~ truncated(Normal(0.1, 0.5), 0.001, 0.8)
    γ ~ truncated(Normal(0.1, 0.5), 0.001, 0.8)
    I_app ~ truncated(Normal(0.5, 0.5), 0.0, 1.0)

    # Simulate Lotka-Volterra model. 
    p = [0.01, α, γ, I_app]
    predicted = solve(prob, Tsit5(); p=p, saveat=ts)

    # Observations.
    data ~ MvNormal(predicted[1,:], σ^2 * I)
    return nothing
end

# ╔═╡ b0ea5bee-8674-4a59-8121-aa1b310490f6
begin
	model = fitlv(spikes_data.v, prob, spikes_data.t)
	
	# Sample 3 independent chains with forward-mode automatic differentiation (the default).
	chain = sample(model, NUTS(0.45), MCMCThreads(), 1000, 10; progress=true)
end

# ╔═╡ 7f9b9241-5675-4ed0-9e10-98dcf29775e9
plot(chain)

# ╔═╡ 33a62cdf-c1a7-4619-8d24-2d7d197b2f4b


# ╔═╡ 0be6eaf0-0abe-4057-9e03-e8c792cd2b52
begin
	plot(; legend=false)
	posterior_samples = sample(chain[[:α,:γ,:I_app]], 200)
	for p in eachrow(Array(posterior_samples))
	    sol_p = solve(prob, Tsit5(); p=[0.01,p...], saveat=0.1)
	    plot!(sol_p; alpha=0.1, color="#BBBBBB")
	end
	scatter!(spikes_data.t, spikes_data.v)
end
...
manifest_format = "2.0"
project_hash = "f6b2f16376095a2778f2bfcf127c1b4e03c618d1"
