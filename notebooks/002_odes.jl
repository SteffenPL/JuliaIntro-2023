...
# ╔═╡ 2d4c169a-b5c7-427d-a545-7459f7f32443
using PlutoUI

# ╔═╡ 6661a5c4-c3de-4c22-b53a-8acda687d53e
using OrdinaryDiffEq

# ╔═╡ c864aa46-f54d-11ed-38d0-e721d9aa5b90
html"""
<style>
	@media screen {
		main {
...
# ╔═╡ 96f38707-de87-4663-857c-34dac72eb4ab
md"""
# Solving differential equations with Julia 🏹

- Julia is currently top-of-the-class when it comes to numerical solution of ODEs (ordinary differential equations).
  - even faster than old FORTRAN codes!
"""

# ╔═╡ 0654c415-0b9b-4769-b7c5-284ad0c27ad2
html"""
<img src="https://docs.sciml.ai/SciMLBenchmarksOutput/stable/MultiLanguage/figures/wrapper_packages_5_1.png" width="600px"></img>
<br>
<b>Figure:</b> Example of bechmarks results <i>(for a very challenging ODE!)</i><br>
"""
...
md"""
## Example 1: The trivial ODE. 😴

Let's start with simulating exponential decay: $\dot x = -\lambda x$.

We will use the Julia package `OrdinaryDiffEq`. 

> - Option 1 🙂 To install Julia packages, one can use the command `using Pkg; Pkg.add("OrdinaryDiffEq")`. 
> - Option 2 😎 Type `]` in the Julia terminal and then `add OrdinaryDiffEq`!
> - In Pluto, installation happens automatically 😇
"""

# ╔═╡ af1af228-eca5-4b0c-89cb-f84aabcf813c
@bind lambda PlutoUI.Slider(-1:0.01:1.0)

# ╔═╡ 2ef755b3-b581-4efe-9b57-c5969928d7dd
params = [lambda]

# ╔═╡ 546389b5-d5ef-4d2f-b887-6aafc6e9212e
prob = ODEProblem( (x, p, t) -> [-x[2], x[1] - p[1] * x[2] ], [1.0, 0.0], (0, 100), params );

# ╔═╡ 0e7aa6db-ba11-45e7-9344-36ebbfe75e12
sol = solve(prob, Euler(), dt = 0.01);

# ╔═╡ d20c20a2-8359-46ea-aed4-771c8ddd44ec
lines(sol[2,:], sol[1,:])
...
md"""
#### Recap: What do we need to define an `ODEProblem`
...
We create a problem via `prob = ODEProblem(rhs, u0, tspan, p)`
and we solve with `sol = solve(prob, Heun())`.

Access solution via 
- `sol.t`: time steps
- `sol[i,:]`: `i`th component of the solution
- `sol(t)`: interpolate state at time `t`
"""
...
(The FitzHugh-Nagumo equations are a simpler model for the Hodgkin-Huxley equations.)

*Equations:*
```math
\varepsilon \frac{d v}{d t} = f(v) - w + I_{\text{app}} 
```
```math
\frac{d w}{dt} = v - \gamma w
```

Let's simulate this equation for $\alpha = 0.1, \gamma = 0.5, \varepsilon = 0.01$ and $f(v) = v(1-v)(v-\alpha)$.
...
# ╔═╡ 21625cec-05be-4798-b341-c9ead96e4b66
f(v, p) = v*(1-v)*(v-p.α)
...
function rhs!(du, u, p, t)
	...
	du[1] = 1/ε * ( f(v, p) - w + I_app )
	du[2] = v - γ * w

	return du
end

# ╔═╡ 2146d152-aed3-49a6-a842-3ba02f820802
@bind x PlutoUI.Slider( LinRange(0,2,101), default = 1 )
...
# ╔═╡ cc857843-b321-4bfa-b255-066629f0a83c
x
...
manifest_format = "2.0"
project_hash = "ddee7db4df19e7a0b54f100a161cb3b7b6db212d"
